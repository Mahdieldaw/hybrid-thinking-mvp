...
This document provides comprehensive information about the data structures used in the Hybrid Thinking platform, including database schemas and TypeScript interfaces.
...
- `userId`: UUID of the user who owns these credentials
- `providerId`: Identifier for the model provider (e.g., "openai", "browser:chatgpt")
- `encryptedPayload`: AES-256-GCM encrypted JSON blob containing `{ accessToken, refreshToken?, expiresAt?, type }`
- `iv`: Initialization vector used for AES-GCM encryption
- `updatedAt`: Timestamp of the last update to this record
...
- `jobId`: Unique identifier for the job (UUID)
- `userId`: UUID of the user who submitted the job
- `workflowName`: Name of the YAML workflow if triggered by one (nullable)
- `promptText`: Original prompt text for direct-prompt jobs (nullable if workflow-driven)
- `requestedModels`: JSON-encoded array of model IDs that were requested
- `results`: JSON-encoded object mapping model IDs to their response text or null
- `synthesisResult`: Final synthesized output text (nullable)
- `status`: Current state of the job ('pending', 'generating', 'synthesizing', 'done', 'failed')
- `createdAt`: Timestamp when the job was created
- `updatedAt`: Timestamp when the job was last updated
- `errorInfo`: Details of any failure (nullable)

## TypeScript "Common Types"

The following TypeScript interfaces define the core data structures used throughout the Hybrid Thinking platform.
...
These TypeScript interfaces define the core data structures used throughout the Hybrid Thinking platform and should be placed in the `packages/core-libs/common-types/src/index.ts` file.
